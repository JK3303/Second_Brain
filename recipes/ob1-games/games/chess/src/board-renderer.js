...
// Opacity per attack count: 1 = low, 2 = medium, 3+ = high
function attackAlpha(count) {
  if (count <= 0) return 0;
...
function render(state, selectedSquare, lastMove, attackMap) {
  _state = state;
  ...
  squares.forEach((sq, i) => {
    ...
    // Attack map overlay
    if (attackMap) {
      const { white: wa, black: ba } = attackMap[i];
      const wAlpha = attackAlpha(wa);
      const bAlpha = attackAlpha(ba);
      ...
      if (wa > 0 || ba > 0) {
        ...
        if (wa > 0 && ba > 0) {
          ...
          overlay.style.background = `rgba(${COLORS.attackBlack},${bAlpha})`;
        }
    ...
    if (_legalTargets.includes(i)) {
      ...
      if (piece !== null) {
        dot.style.cssText = `position:absolute;inset:0;border-radius:50%;box-shadow:inset 0 0 0 4px ${COLORS.legalRing};pointer-events:none;z-index:2;`;
      } else {
        dot.style.cssText = `width:30%;height:30%;border-radius:50%;background:${COLORS.legalDot};pointer-events:none;z-index:2;`;
      }
      sq.appendChild(dot);
    }
...
function renderCaptured(capturedWhite, capturedBlack, whiteEl, blackEl) {
  const renderFn = (pieces, el) => {
    el.textContent = pieces.map(p => UNICODE[p]).join(' ') || '—';
  };
  renderFn(capturedWhite, whiteEl);
  renderFn(capturedBlack, blackEl);
}
